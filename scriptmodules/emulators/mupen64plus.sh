...
function _pkg_info_mupen64plus() {
    ...
    case "$mode" in
        ...
        newer)
            ...
            while read repo; do
                ...
                if [[ -n "${repo[3]}" ]]; then
                    ...
                    if ! hash="$(rp_getRemoteRepoHash git https://ghproxy.com/https://github.com/${repo[0]}/${repo[1]} ${repo[2]})"; then
                        __ERRMSGS+=("$hash")
                        return 3
                    fi
                fi
        ...
        check)
            ...
            while read repo; do
                repo=($repo)
                out=$(rp_getRemoteRepoHash git https://ghproxy.com/https://github.com/${repo[0]}/${repo[1]} ${repo[2]})
                if [[ -z "$out" ]]; then
                    printMsgs "console" "$id repository failed - https://ghproxy.com/https://github.com/${repo[0]}/${repo[1]} ${repo[2]}"
                    ret=1
                fi
            done < <(_get_repos_mupen64plus)
            return "$ret"
...
function sources_mupen64plus() {
    ...
    while read repo; do
        repo=($repo)
        gitPullOrClone "$md_build/${repo[1]}" https://ghproxy.com/https://github.com/${repo[0]}/${repo[1]} ${repo[2]} ${repo[3]}
    done < <(_get_repos_mupen64plus)

    if isPlatform "videocore"; then
        # workaround for shader cache crash issue on Raspbian stretch. See: https://ghproxy.com/https://github.com/gonetz/GLideN64/issues/1665
