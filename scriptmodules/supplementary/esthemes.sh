...
function install_theme_esthemes() {
    ...
    if [[ -z "$branch" ]]; then
        ...
        branch=$(runCmd git ls-remote --symref --exit-code "https://ghproxy.com/https://github.com/$repo/es-theme-$theme.git" HEAD | grep -oP ".*/\K[^\t]+")
    ...
    gitPullOrClone "/etc/emulationstation/themes/$name" "https://ghproxy.com/https://github.com/$repo/es-theme-$theme.git" "$branch"
}
                ...
                if [[ "${status[0]}" == "i" ]]; then
                    ...
                    case "$choice" in
                        1)
                            ...
                            ;;
                        2)
                            gitPullOrClone "$gallerydir" "https://ghproxy.com/https://github.com/wetriner/es-theme-gallery"
                            ;;
                        3)
                            if [[ -d "$gallerydir" ]]; then
                                rm -rf "$gallerydir"
